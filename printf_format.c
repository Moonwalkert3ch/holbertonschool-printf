...
int selector(const char *format, va_list args, int printed_chars)
{
	...
	case 'd':
	case 'i':
	printed_chars = printf_integer(args, printed_chars);
	break;
	case 'c':
	...
	case 'x':
	case 'X':
	printed_chars = _x(va_arg(args, unsigned int), printed_chars, (*format == 'X') ? 1 : 0);
	break;
	case 'o':
	printed_chars = printf_octal(va_arg(args, unsigned int), printed_chars);
	break;
	case 'u':
